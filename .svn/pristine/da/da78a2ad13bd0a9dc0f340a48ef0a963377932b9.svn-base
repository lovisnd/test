...
public class UserService implements IUserService {
		...
		if (userLogin != null) {
			if (userLogin.getEnabled() == 0) {
				...
				try {
					...
					String token = MD5Util.md5(uid, UUIDUtil.getUUID());
					userLogin.setToken(token);
					userLogin.setTokenTime(new Date().getTime());
	...
	public List<User> getExportUser(Map<String, Object> map) {
		...
		for (User user : userList) {
			...
			if (!roleList.isEmpty()) {
				...
				rolesName = roles.substring(0, roles.toString().lastIndexOf(","));
			}
	...
	public int resetPassword(String id) {
		...
		hashMap.put("pwdUpdateTime", new Date());
